"use client";
import { cn } from "@/lib/utils";
import { MutableRefObject, useEffect, useRef, useState } from "react";

// Hook personnalisé useClickOutside
const useClickOutside = (handler: () => void): MutableRefObject<HTMLDivElement | null> => {
  const domNode = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const maybeHandler = (event: MouseEvent) => {
      if (domNode.current && !domNode.current.contains(event.target as Node)) {
        handler();
      }
    };
    document.addEventListener("mousedown", maybeHandler);

    return () => {
      document.removeEventListener("mousedown", maybeHandler);
    };
  }, [handler]);

  return domNode;
};
interface Props {
  videoLink: string;
  imageLink: string;
  backgroundButtonClassName?: string; // className
  foregroundButtonClassName?: string; // className
}

export default function VideoUI({ videoLink, imageLink, backgroundButtonClassName, foregroundButtonClassName }: Props) {
  const [modalOpen, setModalOpen] = useState(false);

  const domNode = useClickOutside(() => {
    setModalOpen(false);
  });

  useEffect(() => {
    const handleEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        setModalOpen(false);
      }
    };

    document.addEventListener('keydown', handleEscape);

    return () => {
      document.removeEventListener('keydown', handleEscape);
    };
  }, []);

  useEffect(() => {
    const videoElement = domNode.current?.querySelector('video');
    if (modalOpen && videoElement) {
      videoElement.play().catch((error) => {
        console.error('Error attempting to play', error);
      });
    } else if (videoElement) {
      videoElement.pause();
    }
  }, [modalOpen, domNode]);

  return (
    <div className="flex justify-center mx-auto">
      {/* Video thumbnail */}
      <button
        className="relative flex justify-center items-center focus:outline-none focus-visible:ring focus-visible:ring-indigo-300 rounded-3xl group"
        onClick={() => setModalOpen(true)}
        aria-controls="modal"
        aria-label="Watch the video"
      >
        <img
          className="rounded-3xl shadow-2xl transition-shadow duration-300 ease-in-out"
          src={imageLink}
          width="768"
          height="432"
          alt="Modal video thumbnail"
        />
        {/* Play icon */}
        <svg
          className="absolute pointer-events-none group-hover:scale-110 transition-transform duration-300 ease-in-out"
          xmlns="http://www.w3.org/2000/svg"
          width="72"
          height="72"
        >
          <circle className={cn("fill-white", backgroundButtonClassName)} cx="36" cy="36" r="36" fillOpacity=".8" />
          <path
            className={cn("fill-indigo-500 drop-shadow-2xl", foregroundButtonClassName)}
            d="M44 36a.999.999 0 0 0-.427-.82l-10-7A1 1 0 0 0 32 29V43a.999.999 0 0 0 1.573.82l10-7A.995.995 0 0 0 44 36V36c0 .001 0 .001 0 0Z"
          />
        </svg>
      </button>
      {/* End: Video thumbnail */}

      {/* Modal backdrop */}
      {modalOpen && (
        <div
          className="fixed inset-0 z-[99999] bg-black bg-opacity-50 transition-opacity"
          onClick={() => setModalOpen(false)}
          aria-hidden="true"
        />
      )}
      {/* End: Modal backdrop */}

      {/* Modal dialog */}
      {modalOpen && (
        <div
          id="modal"
          className="fixed inset-0 z-[99999] flex px-4 md:px-6 py-6"
          role="dialog"
          aria-modal="true"
        >
          <div className="max-w-5xl mx-auto h-full flex items-center">
            <div
              className="w-full max-h-full rounded-3xl shadow-2xl aspect-video bg-black overflow-hidden"
              ref={domNode}
            >
              <video width="1920" height="1080" loop controls controlsList="nodownload">
                <source
                  src={videoLink}
                  type="video/mp4"
                />
                Your browser does not support the video tag.
              </video>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
